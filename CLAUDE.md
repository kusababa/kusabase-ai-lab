...
- **`ai-news-pipeline.yml`の「Commit collection log & dedup ledger to main」ステップは単発の`git push`ではなく、失敗時にfetch→rebase→再pushをリトライする構成にしてある。** 同じワークフロー実行を手動で再実行（rerun）すると、直前の実行が既にpush済みのコミットとmainの先頭がズレて`! [rejected] main -> main (fetch first)`でジョブごと失敗する事象が実際に発生したため。なお、この用途での再実行自体は基本的に不要（`[generate] 下書き生成: 0件`はスコア70点未満のニュースしか無かっただけの正常系であり、不具合ではない）
